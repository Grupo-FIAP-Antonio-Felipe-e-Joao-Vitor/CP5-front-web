...
const EditarPlano = ({ usuario, token, setToken, setUsuario }) => {
    const location = useLocation();
    const navigate = useNavigate();
    const { dados } = location.state || {}; // pega os dados enviados
    const { register, handleSubmit, reset } = useForm({
        defaultValues: {
            nome: dados?.nome || "",
            preco: dados?.preco || "",
            tempo: dados?.tempo || "",
            musculacao: dados?.beneficios?.find(b => b.nome === "musculacao")?.ativo || false,
            crossfit: dados?.beneficios?.find(b => b.nome === "crossfit")?.ativo || false,
            funcional: dados?.beneficios?.find(b => b.nome === "funcional")?.ativo || false,
            danca: dados?.beneficios?.find(b => b.nome === "danca")?.ativo || false,
            pilates: dados?.beneficios?.find(b => b.nome === "pilates")?.ativo || false,
            personal: dados?.beneficios?.find(b => b.nome === "personal")?.ativo || false,
        },
    });
    ...
    async function editar(data) {
        try {
            ...
            const response = await axios.put(`${url}/${dados.id}`, planoAtualizado, {
                headers: {
